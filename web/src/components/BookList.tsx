...
export function BookList({ view, books, library, loading, error, total, page, onPage, onSelect, onFavorite }: Props) {
  ...
  return <section className="content">
    <div className="content-heading"><div>{view !== "library" && <p>{t("searchResults")}</p>}<h1>{view === "library" ? t("library") : loading ? t("searching") : `${total.toLocaleString(language === "sv" ? "sv-SE" : "en-US")} ${t("hits")}`}</h1></div>{view === "library" && <Library />}</div>
