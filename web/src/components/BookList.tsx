...
export function BookList({ view, books, library, loading, error, total, page, onPage, onSelect, onFavorite }: Props) {
  ...
  return <section className="content">
    ...
    {!loading && <div className="book-list">{shown.map(book => {const saved=owned(book.id);return <article className="book-row" key={book.id} onClick={() => onSelect(saved || book)}><div className="cover">{book.cover?<img src={book.cover} alt={`${t("coverOf")} ${book.title}`}/>:<BookOpen/>}</div><div className="book-copy"><h2>{book.title}</h2><p>{book.authors.join(", ")}</p><span>{book.year || t("unknownYear")}{book.pages?` · ${book.pages} ${t("pages")}`:""}</span>{view === "library" && <ReadingStatusBadge status={book.readingStatus}/>}</div><button className={`heart ${saved?.favorite?"active":""}`} onClick={event=>{event.stopPropagation();onFavorite(book)}} aria-label={t("favorite")}><Heart fill={saved?.favorite?"currentColor":"none"}/></button>{saved&&<span className="owned" title={t("bookInLibrary")}><Check/></span>}</article>})}</div>}
