...
export function DiscoverPage({entries,loading,onSelect}:Props){
  ...
  const books=useMemo(()=>{
    ...
    entries.forEach(book=>{
      if(scope==="owned"&&book.owned===false)return;
      if(scope==="read"&&book.readingStatus!=="read")return;
      if(genre!=="all"&&!book.genres?.includes(genre))return;
      if(term&&!`${book.title} ${book.authors.join(" ")}`.toLocaleLowerCase().includes(term.toLocaleLowerCase()))return;
      const item=map.get(book.id)||{book,count:0,ratings:[]};
      item.count++;
      if(typeof book.rating==="number")item.ratings.push(book.rating);
      map.set(book.id,item);
    });
    return Array.from(map.values()).filter(item=>{
      if(minRating===0)return true;
      if(item.ratings.length===0)return false;
      return item.ratings.reduce((x,y)=>x+y,0)/item.ratings.length>=minRating;
    }).sort((a,b)=>{
      const ar=a.ratings.length?a.ratings.reduce((x,y)=>x+y,0)/a.ratings.length:0;
      const br=b.ratings.length?b.ratings.reduce((x,y)=>x+y,0)/b.ratings.length:0;
  ...
  return <section className="content discover-page">
    ...
    <div className="discover-filters">
      <input value={term} onChange={e=>setTerm(e.target.value)} placeholder={t("searchPlaceholder")}/>
      <select value={scope} onChange={e=>setScope(e.target.value)}><option value="all">{t("allBooks")}</option><option value="owned">{t("totalOwned")}</option><option value="read">{t("read")}</option></select>
