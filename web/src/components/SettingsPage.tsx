...
export function SettingsPage({theme,setTheme,user,books,shelves,installMode,onInstall,isAdmin,userCount,onCreateShelf,onDeleteShelf,onImport,onDeleteLibrary,onDeleteAccount}:Props){
  ...
  return <section className="content settings-page"><div className="content-heading"><div><p>{t("customize")}</p><h1>{t("settings")}</h1></div><Settings/></div>
    ...
    <div className="settings-card shelves-settings"><h2>{t("myShelves")}</h2><p>{t("shelfHelp")}</p><div className="tag-input"><input value={shelfName} onChange={event=>setShelfName(event.target.value)} onKeyDown={event=>{if(event.key==="Enter")void createShelf()}} placeholder={t("shelfName")}/><button onClick={()=>void createShelf()}><Plus/>{t("createShelf")}</button></div>{shelves.filter(shelf=>shelf.custom).length>0&&<div className="shelf-manage-list">{shelves.filter(shelf=>shelf.custom).map(shelf=><div key={shelf.id}><span>{shelf.name}</span><button onClick={()=>void onDeleteShelf(shelf)} aria-label={`${t("deleteShelf")}: ${shelf.name}`}><Trash2/></button></div>)}</div>}</div>
    <div className="settings-card install-card"><Smartphone/><div><h2>{t("installSunBooks")}</h2><p>{t("installHelp")}</p>{installMode==="installed"?<strong>{t("installed")}</strong>:installMode==="ios"?<span>{t("iosInstall")}</span>:installMode==="available"?<button onClick={onInstall}><Download/>{t("installButton")}</button>:<span>{t("installUnavailable")}</span>}</div></div>
